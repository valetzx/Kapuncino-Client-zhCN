...
public class AntiCrash extends Module {
    ...
    @Getter
    BooleanSetting hideMassEntities = this.config.create(new BooleanSetting.Builder(true).name("Smart hide")
        .description("Hides entities when there's too many of them automatically\nDoes not apply to players")
        .get());

    @Getter
    DoubleSetting massEntityAmount = this.config.create(new DoubleSetting.Builder(40).name("Entity amount")
        .description("The amount of entities to consider \"too many\"")
        .min(1)
        .max(100)
        .precision(0)
        .get());
    ...
    public AntiCrash() {
        ...
        massEntityAmount.showIf(hideMassEntities::getValue);
    }
