...
public class AutoTrap extends Module {
    static final double[][] buildOffsetsSmall = new double[][] { new double[] { 0, 2, 0 }, new double[] { 1, 1, 0 }, new double[] { 0, 1, 1 }, new double[] { -1, 1, 0 },
        new double[] { 0, 1, -1 }, new double[] { 0, -1, 0 } };
    ...
    boolean isTrappedAlready(Entity entity) {
        ...
        BlockPos bp = new BlockPos(BlockPos.ofFloored(entityPos));
        ...
        double[][] possibleOffsetsHome = new double[][] { new double[] { 1, 0, 1 }, new double[] { 1, 0, 0 }, new double[] { 1, 0, -1 }, new double[] { 0, 0, -1 },
            new double[] { -1, 0, -1 }, new double[] { -1, 0, 0 }, new double[] { -1, 0, 1 }, new double[] { 0, 0, 1 }, new double[] { 0, 0, 0 } };
        for (double[] ints : possibleOffsetsHome) {
            ...
            boolean matches = Arrays.stream(buildOffsetsBig).allMatch(ints1 -> {
                BlockPos a = new BlockPos(BlockPos.ofFloored(potentialHome.add(ints1[0], ints1[1], ints1[2])));
    ...
    public void onFastTick() {
        for (Entity player : Objects.requireNonNull(CoffeeMain.client.world).getPlayers()) {
            ...
            BlockPos bp = new BlockPos(BlockPos.ofFloored(pos));
            double eWidth = player.getWidth();
            BlockPos corner = new BlockPos(BlockPos.ofFloored(pos.subtract(eWidth / 2, 0, eWidth / 2)));
            BlockPos otherCorner = new BlockPos(BlockPos.ofFloored(pos.add(eWidth / 2, 0, eWidth / 2)));
            double[][] planToUse;
            ...
            CoffeeMain.client.execute(() -> {
                ...
                for (double[] ints : filteredPlan) {
                    BlockPos current = new BlockPos(BlockPos.ofFloored(pos.add(ints[0], ints[1], ints[2])));
